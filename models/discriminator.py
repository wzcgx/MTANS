...
class GlobalConvBlock(nn.Module):
    def __init__(self, in_dim, out_dim, kernel_size):
        ...
        self.conv_l1 = nn.Conv3d(in_dim, out_dim, kernel_size=(kernel_size[0], 1),
                                 padding=(pad0, 0))
        self.conv_l2 = nn.Conv3d(out_dim, out_dim, kernel_size=(1, kernel_size[1]),
                                 padding=(0, pad1))
        self.conv_r1 = nn.Conv3d(in_dim, out_dim, kernel_size=(1, kernel_size[1]),
                                 padding=(0, pad1))
        self.conv_r2 = nn.Conv3d(out_dim, out_dim, kernel_size=(kernel_size[0], 1),
                                 padding=(pad0, 0))
...
class NetS(nn.Module):
    def __init__(self, ngpu):
        ...
        self.convblock1 = nn.Sequential(
            ...
            nn.Conv3d(channel_dim, ndf, 7, 2, 3, bias=False),
        ...
        self.convblock2 = nn.Sequential(
            # state size. (ndf) x 64 x 64
            nn.Conv3d(ndf, ndf * 2, 5, 2, 2, bias=False),
        ...
        self.convblock3 = nn.Sequential(
            # state size. (ndf*2) x 32 x 32
            nn.Conv3d(ndf * 2, ndf * 4, 5, 2, 2, bias=False),
        ...
        self.convblock4 = nn.Sequential(
            # state size. (ndf*4) x 16 x 16
            nn.Conv3d(ndf * 4, ndf * 8, 5, 2, 2, bias=False),
        ...
        self.convblock5 = nn.Sequential(
            # state size. (ndf*8) x 8 x 8
            nn.Conv3d(ndf * 8, ndf * 8, 5, 2, 2, bias=False),
            nn.BatchNorm3d(ndf * 8),
            nn.LeakyReLU(0.2, inplace=True),
        ...
        self.convblock6_1 = nn.Sequential(
            # state size. (ndf*16) x 2 x 2
            nn.Conv3d(ndf * 16, ndf * 16, kernel_size=1, bias=False),
            nn.BatchNorm3d(ndf * 16),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (ndf*16) x 2 x 2
        )
        self.convblock7 = nn.Sequential(
            # state size. (ndf*16) x 2 x 2
            nn.Conv3d(ndf * 16, ndf * 32, 3, 2, 1, bias=False),
        ...
        self.convblock8 = nn.Sequential(
            # state size. (ndf*32) x 1 x 1
            nn.Conv3d(ndf * 32, ndf * 8, kernel_size=1, bias=False),
            nn.BatchNorm3d(ndf * 8),
            nn.LeakyReLU(0.2, inplace=True),
        ...
        self.deconvblock1 = nn.Sequential(
            ...
            nn.ConvTranspose3d(ndf * 8, ndf * 32, kernel_size=1, bias=False),
            nn.BatchNorm3d(ndf * 32),
            nn.ReLU(True),
        ...
        self.deconvblock2 = nn.Sequential(
            ...
            nn.Conv3d(ndf * 64 , ndf * 16, 3, 1, 1, bias=False),
            nn.BatchNorm3d(ndf * 16),
            nn.ReLU(True),
        ...
        self.deconvblock3 = nn.Sequential(
            ...
            nn.Conv3d(ndf * 16 * 2, ndf * 8, 3, 1, 1, bias=False),
            nn.BatchNorm3d(ndf * 8),
            nn.ReLU(True),
        ...
        self.deconvblock4 = nn.Sequential(
            ...
            # nn.ConvTranspose3d(ndf * 8 * 2, ndf * 8, 4, 2, 1, bias=False),
            nn.BatchNorm3d(ndf * 8),
            nn.ReLU(True),
        ...
        self.deconvblock5 = nn.Sequential(
            ...
            # nn.ConvTranspose3d(ndf * 8 * 2, ndf * 4, 4, 2, 1, bias=False),
            nn.BatchNorm3d(ndf * 4),
            nn.ReLU(True),
        ...
        self.deconvblock6 = nn.Sequential(
            ...
            # nn.ConvTranspose3d(ndf * 4 * 2, ndf * 2, 4, 2, 1, bias=False),
            nn.BatchNorm3d(ndf * 2),
            nn.ReLU(True),
        ...
        self.deconvblock7 = nn.Sequential(
            ...
            # nn.ConvTranspose3d(ndf * 2 * 2,     ndf, 4, 2, 1, bias=False),
            nn.BatchNorm3d(ndf),
            nn.ReLU(True),
        ...
        self.deconvblock9 = nn.Sequential(
            # state size. (ngf) x 128 x 128
            nn.Conv3d( ndf, 1, 5, 1, 2, bias=False),
    ...
    def forward(self, input):
        ...
        if isinstance(input.data, torch.cuda.FloatTensor) and self.ngpu is 1:
            ...
            decoder1 = F.upsample(decoder1, size = encoder6.size()[2:], mode='trilinear')
...
class NetC(nn.Module):
    def __init__(self, ngpu):
        ...
        self.convblock0 = nn.Sequential(
            # input is (channel_dim) x 128 x 128
            nn.Conv3d(num_classes, ndf, 7, 2, 3, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
            # nn.Dropout3d(p=0.2),
            # state size. (ndf) x 64 x 64
        )
        ...
        self.convblock2 = nn.Sequential(
            # state size. (ndf * 2) x 64 x 64
            nn.Conv3d(ndf * 1, ndf * 2, 5, 2, 2, bias=False),
            nn.BatchNorm3d(ndf * 2),
            nn.LeakyReLU(0.2, inplace=True),
            # nn.Dropout3d(p=0.2),
            # state size. (ndf*2) x 32 x 32
        ...
        self.convblock5 = nn.Sequential(
            # state size. (ndf*8) x 8 x 8
            nn.Conv3d(ndf * 8, ndf * 8, 4, 2, 1, bias=False),
            nn.BatchNorm3d(ndf * 8),
            nn.LeakyReLU(0.2, inplace=True),
            # nn.Dropout3d(p=0.2),
        ...
        self.convblock5_1 = nn.Sequential(
            ...
            nn.Conv3d(ndf * 16, ndf * 32, 3, 1, 1, bias=False),
            nn.BatchNorm3d(ndf * 32),
            nn.LeakyReLU(0.2, inplace=True),
            # nn.Dropout3d(p=0.2),
        ...
        self.convblock6 = nn.Sequential(
            # state size. (ndf*32) x 4 x 4
            nn.Conv3d(ndf * 8, ndf * 8, 3, 2, 1, bias=False),
            nn.BatchNorm3d(ndf * 8),
            nn.LeakyReLU(0.2, inplace=True),
            # nn.Dropout3d(p=0.2),
        ...
        self.avgpool_1 = nn.AvgPool3d((7, 7, 7))    
    ...
    def forward(self, map, input):
        if isinstance(input.data, torch.cuda.FloatTensor) and self.ngpu is 1:
            batchsize = input.size()[0]

            map_feature = self.convblock0(map)
            image_feature = self.convblock1(input)


            image = torch.add(map_feature, image_feature)            
            # out1 = self.convblock1(image)
            # out1 = self.convblock1_1(out1)
            out2 = self.convblock2(image)
            ...
            map = self.avgpool_1(out4)
            out = map.view(batchsize, -1)
            D_out = self.classifier(out)
            DD_out = D_out.reshape((batchsize, 2))
            ...
            output = torch.cat((image.view(batchsize,-1),
